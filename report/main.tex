...
F1~=~0.691 on AD (Logistic Regression, 184 features) and F1~=~0.420 on FC
(Oracle K\,=\,4 hierarchical grouping), versus the published DL figures of 0.799
and 0.623 respectively---figures we treat as indicative only, since the source
preprint was subsequently withdrawn by its authors over metric-computation flaws.
The analysis shows that global statistical features are competitive for binary
detection, but a sizeable gap persists on FC because fault signatures concentrate
in the 12th--18th principal components, contributing less than 0.1\% of total
variance and remaining largely invisible to global statistics. The end-to-end
pipeline achieves F1~=~0.184, limited primarily by Stage~2 fault classification
errors (40.6\% of all errors).
\end{abstract}
...
\newpage
\input{chapters/ch2_dataset.tex}
\newpage
\input{chapters/ch3_features.tex}
\newpage
...
\newpage
\input{chapters/ch7_summary.tex}
